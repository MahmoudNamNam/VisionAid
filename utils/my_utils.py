...
def generate_speech(objects, lang="en"):
    ...
    Args:
        ...
        lang (str, optional): The language for speech generation ('en', 'ar', or 'de'). Defaults to "en".
    ...
    if not objects:
        ...
        if lang == "ar":
            ...
            translated_objects = [german_translations.get(obj, obj) for obj in objects]
            if len(translated_objects) == 1:
                sentence = f"Ich sehe {translated_objects[0]}"
            else:
                sentence = f"Ich sehe {', '.join(translated_objects[:-1])} und {translated_objects[-1]}"
        else:
            if len(objects) == 1:
                sentence = f"I see {objects[0]}"
            else:
                sentence = f"I see {', '.join(objects[:-1])} and {objects[-1]}"
...
if __name__ == '__main__':
    ...
    objects_de_single = ["person"]
    audio_de_single_base64 = generate_speech(objects_de_single, "de")
    print("German Audio (Single Object - Base64):", audio_de_single_base64[:50] + "...")

    objects_de_multiple = ["person", "car", "chair"]
    audio_de_multiple_base64 = generate_speech(objects_de_multiple, "de")
    print("German Audio (Multiple Objects - Base64):", audio_de_multiple_base64[:50] + "...")
    ...
    audio_empty_de = generate_speech(empty_objects, "de")
    print("Empty German Audio (Base64):", audio_empty_de)